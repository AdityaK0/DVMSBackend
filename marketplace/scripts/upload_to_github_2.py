...
def upload_to_github(image_url, vendor_id, collection_id, filename):
    ...
    try:
        ...
        github_path = f"{BASE_FOLDER}/vendor_{vendor_id}/product_{collection_id}/{filename}"
        upload_url = f"{API_BASE_URL}/{github_path}"

        # 4️⃣ Make API request to GitHub
        data = {
            "message": f"Add {filename} for vendor {vendor_id}, product {collection_id}",
...
def migrate_images_to_github():
    ...
    images = PortfolioCollection.objects.filter(is_active=True)
    print(f"Found {images.count()} images to process...")

    for img in images:
        if not img.cover_image.url:
        ...
        new_url = upload_to_github(img.image.url, vendor_id, collection_id, filename)
